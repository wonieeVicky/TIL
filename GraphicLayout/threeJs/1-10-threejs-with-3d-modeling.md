...
우선 새로운 블렌더 파일 오픈, Camera, Light는 사용하지 않으므로 삭제
EditMode로 이동 후 S + x, S + y를 통해 도미노 모양으로 만들어 줌

![](../../img/230420-1.png)

다음 숫자 2를 누른 뒤 엣지 선택모드로 이동 후 모서리 네 지점을 중복 선택 후 Ctrl + B를 한다.

![](../../img/230420-1.gif)

위와 같이 선택 후 Ctrl + B + 스크롤하면 조각을 나눌 수 있음. 이번에는 inset이란 기능을 구현해보자.

![](../../img/230420-2.gif)

면 선택 후 단축키 i를 누른 뒤 스크롤을 조정하면 새로운 면이 생성. 앞 뒤 중복으로 선택해서 똑같이 생성한다.
이 부분을 안쪽으로 들어가게 만들어보자. 면을 선택 후 E를 누른 다음 마우스로 조정해주면 된다.

![](../../img/230420-2.png)

Object 모드로 빠져나온 다음 색을 칠해준다. 좌측에 창을 추가 생성 후 Uv Editor 오픈

![](../../img/230420-3.png)

정상적으로 전개도가 그려졌으나 기본적으로 Average Islands Scale, Pack Islands는 한번씩 해준다.

![](../../img/230420-4.png)

위와 같이 정리됨. 이제 UV Editor → Image Editor 변경 → Paint 모드 변경 후 상단 탭에 Texture Paint 클릭

![](../../img/230420-5.png)

다음 기본 색을 Base Color로 추가해준다. 노란색으로 넣어줄 것임

![](../../img/230420-6.png)

대충 그림을 그린 다음 export 해준다.

![](../../img/230420-3.gif)

File → Export → glTF 2.0 클릭. 옵션은 Include → Selected Objects 선택

이제 만든 도미노를 다음 시간에 적극적으로 활용해본다.
