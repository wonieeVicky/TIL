...
이번에는 불필요한 css를 제거하여 성능을 최적화해보려고 한다. 먼저 lightHouse로 현재 성능을 확인해본다.

![](../../img/220914-1.png)

위처럼 Opportunity로 사용하지 않는 CSS를 줄이라는 문구가 나오고 있다. 실제 줄일 수 있는 크기를 확인해보면 실제적으로는 사용하는 CSS가 10KB 남짓 밖에 되지 않는다는 것을 확인할 수 있다.

위 내용을 좀 더 자세히 확인하기 위해서 console 창에서 Coverage 창을 활성화하여 refresh 해보면 내역을 아래와 같이 확인해볼 수 있다.

![](../../img/220914-2.png)

빨간색 그래프가 클수록 사용되지 않는 리소스들이 많은 것을 의미하며, CSS의 경우 99.7%가 사용되지 않는 파일임을 알 수 있다. 위 css 파일을 더블클릭하면 실제 사용에 대한 정보를 UI로 확인할 수 있다.

![빨간색은 unused, 파란색은 used](../../img/220914-3.png)

사용하지 않는 스타일 코드에 대해 제외하는 방법으로는 여러가지가 있는데 우리는 PurgeCSS를 사용하여 static한 HTML 내에 존재하는 클래스나 태그 정보를 비교하여 실제 사용하는 스타일 코드만 남기는 라이브러리이다.

```jsx
> npm i purgecss --save-dev
```

위와 같이 purgecss를 설치해준 뒤 package.json 내 scripts 명령어를 추가해준다.
...
위와 같이 설정 후 빌드된ㄷcss를 purgeCSS로 변환해주면 현격하게 작은 사이즈로 파일이 변경된 것을 확인할 수 있다! 하지만, 반응형 스타일이 모두 적용되어 있지 않는 문제가 발생함. 스타일 클래스 내부에 `:` 등의 특수문자가 포함되어서 그런 것인데, 해당 특수문자를 purgeCSS의 경우 나눠서 인식하기 때문이다.

따라서 이러한 이슈들이 발생하는 클래스를 정규표현식으로 걸러주도록 config를 작성해본다.
...
위에서 `\w`란 `0-9a-zA-Z_`를 의미한다. (참고) 위 내용을 package.json에 아래와 같이 반영해줌
...
위 과정으로 새롭게 서버를 띄워보면, css파일 크기가 10KB 남짓하게 줄여져서 빌드된 것을 확인할 수 있다.
