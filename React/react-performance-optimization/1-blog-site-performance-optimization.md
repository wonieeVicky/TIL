...
LightHouse 툴을 이용한 페이지 검사 → 이미지 사이즈 최적화 → Bottleneck 코드 최적화 → Code Splitting → 텍스트 압축의 과정으로 블로그 사이트 최적화를 해보았다. 전체적으로 페이지에 대한 검사를 진행함으로써 다음 단계가 진행된 것이다. 병목을 일으키는 리소스와 함수 등을 직접 탭에서 찾아내고, 사용하지 않는 모듈을 적절히 나누는 것도 배워보았다. 또한 서비스 최종 배포 시 빌드하고 리소스를 압축하는 것도 배워보았다.

이처럼 단순히 페이지 개발만 하는 것이 아닌 report에 대한 적절한 분서과 개선을 해나감으로서 서비스의 효율성까지 고려할 줄 아는 개발자로 성장해보는 것이 좋겠다 🥸
